...
bietschhorn_week <- rbind(bietschhorn_monday,bietschhorn_tuesday, 
                          ...
                          bietschhorn_sunday)
bietschhorn_week <- bietschhorn_week %>% as_tsibble(key = Types)

# Plotting from Monday till Wednesday 
bietschhorn_week123 <- bietschhorn_week %>% 
  filter(weekday == "Monday" |weekday == "Tuesday" | 
           weekday == "Wednesday")

# plot the data
qplot(date, Number, data = bietschhorn_week123, geom = "line", 
      group = Types, colour= Types, main = "Bike usage per day", 
      xlab = "Date [10 min]", ylab = "Number of bike") +
  facet_grid(weekday ~ ., scale = "free_y")

# Plotting from Thursday till Sunday 
bietschhorn_week4567 <- bietschhorn_week %>% 
  filter(weekday == "Thursday" |weekday == "Friday" | 
           weekday == "Saturday" | weekday == "Sunday")

# plot the data
qplot(date, Number, data = bietschhorn_week4567, geom = "line", 
      group = Types, colour= Types, main = "Bike usage per day", 
      xlab = "Date [10 min]", ylab = "Number of bike") +
  facet_grid(weekday ~ ., scale = "free_y")
```
