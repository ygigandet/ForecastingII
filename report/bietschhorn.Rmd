...
scenario <- new_data(bietschhorn_wednesdays_bike, n = 144) %>% 
...
# TSLM model
tslm_model_bike <- bietschhorn_wednesdays_bike %>% 
  model(TSLM = TSLM(Number ~ Air_Temperature + Precipitation +
                      Virtual_Temperature + Wind_Speed))

fc_scenario <- fabletools::forecast(tslm_model_bike, new_data = scenario)

bietschhorn_wednesdays_bike %>% autoplot(Number) +
  autolayer(fc_scenario, level = 0)
```
