...
# ACF plots to determine autocorrelation on the tables
bietschhorn %>%
  filter(Types == "Bike" | Types == "E-Bike") %>%
  ACF(Number, lag_max = 144) %>% # There are 144 intervals of 10 min in a day
   autoplot() +
  labs(title="Usage frequencies in Bietschhorn station")
...
p1 <- bietschhorn %>% filter(Types != "Capacity") %>% ACF(Number) %>% autoplot()
p2 <- bietschhorn %>% filter(Types != "Capacity") %>% ACF(diff_number) %>% autoplot()
p1 + p2

fit <- bietschhorn %>% filter(Types == "Bike") %>% model(ARIMA(Number)) 
                                                  # Model: ARIMA(1,1,0)(0,0,2)[6] 
