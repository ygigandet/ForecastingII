...
blancherie %>% 
  ...
  autoplot(Number) +
  xlab("Date") + ylab("Number of bike") +
  ggtitle("Usage frequencies in Blancherie")
```
...
```{r, echo=FALSE, message=FALSE}
# Plotting from Monday till Wednesday every 10 min difference
blacherie123_10 <- blancherie %>% 
  filter(weekday == "Monday" |weekday == "Tuesday" | 
           weekday == "Wednesday") %>%
  filter(minutes == 10)

# plot the data
qplot(date, Number, data = blacherie123_10, geom = "line", 
      group = Types, colour= Types, main = "Bike usage per day", 
      xlab = "Date [10 min]", ylab = "Number of bike") +
  facet_grid(weekday ~ ., scale = "free_y")

# Plotting from Thursday till Sunday every 10 min difference
blacherie4567_10 <- blancherie %>% 
  filter(weekday == "Thursday" |weekday == "Friday" | 
           weekday == "Saturday" | weekday == "Sunday") %>%
  filter(minutes == 10)

# plot the data
qplot(date, Number, data = blacherie4567_10, geom = "line", 
      group = Types, colour= Types, main = "Bike usage per day", 
      xlab = "Date [10 min]", ylab = "Number of bike") +
  facet_grid(weekday ~ ., scale = "free_y")
```

Monday - One obs with 120 min diff... previous obs have the same amount of types of bikes
```{r, echo=FALSE, message=FALSE}
...
Wednesday - The Observations with time difference as 40, 30 & 20min are only repeated once on this day.
...
Sunday - One obs with 70 min diff... previous and following obs have the same amount of types of bikes
```{r, echo=FALSE, message=FALSE}
blancherie_7 <- blancherie_ts %>% 
  filter(weekday == "Sunday")
unique(blancherie_7[c("minutes")]) # Identify the unique values of a column
```

```{r, echo=FALSE, message=FALSE}
# Plotting from Monday till Wednesday every 60 min difference
blacherie123_60 <- blancherie %>% 
  filter(weekday == "Monday" |weekday == "Tuesday" | 
           weekday == "Wednesday") %>%
  filter(minutes == 60)

# plot the data
qplot(date, Number, data = blacherie123_60, geom = "line", 
      group = Types, colour= Types, main = "Bike usage per day", 
      xlab = "Date [60 min]", ylab = "Number of bike") +
  facet_grid(weekday ~ ., scale = "free_y")

# Plotting from Thursday till Sunday every 60 min difference
blacherie4567_60 <- blancherie %>% 
  filter(weekday == "Thursday" |weekday == "Friday" | 
           weekday == "Saturday" | weekday == "Sunday") %>%
  filter(minutes == 60)

# plot the data
qplot(date, Number, data = blacherie4567_60, geom = "line", 
      group = Types, colour= Types, main = "Bike usage per day", 
      xlab = "Date [60 min]", ylab = "Number of bike") +
  facet_grid(weekday ~ ., scale = "free_y")
```

We can observe on the previous graph that Thursday is missing, it is because the daily time series is divided only by 10 minutes difference.

> Graphs with 10 and 60 minutes difference

```{r, echo=FALSE, message=FALSE}
# Plot the complete week by 10 minute periods
blacherie_10 <- blancherie %>% 
  filter(minutes == 10)
# plot the data
p1 <- blacherie_10 %>%
  autoplot(Number) +
...
# Plot the complete week by 1hour periods
blacherie_60 <- blancherie %>% 
  filter(minutes == 60)
# plot the data
p2 <-blacherie_60 %>%
  autoplot(Number) +
...
blancherie_6 <- blancherie_ts %>% 
  filter(weekday == "Saturday")
unique(blancherie_6[c("minutes")]) # Identify the unique values of a column
