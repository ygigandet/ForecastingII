...
fit <- bietschhorn_mondays %>%
  model(ETS(Number)) # Model: ETS(M,N,N) 
fc <- fit %>%
  forecast(h = 144)
p1 <- fc %>%
  autoplot(bietschhorn_mondays) +
  xlab("Timelaps per Monday") + ylab("Number of bike") +
...
bietschhorn_mondays_tuesdays <- bietschhorn_mondays_tuesdays %>% 
  mutate(t = row_number()) %>% 
  update_tsibble(index = t, regular = TRUE)

bietschhorn_mondays_tuesdays %>% 
  autoplot(Number)
...
fc %>%
  autoplot(bietschhorn_mondays_tuesdays)
