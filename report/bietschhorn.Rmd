...
bietschhorn_wednesdays_bikes %>% features(Number, unitroot_kpss) 
...
fit <- bietschhorn_wednesdays_bikes[1:interval,] %>%
...
fit <- bietschhorn_wednesdays_bikes_fc %>%
...
fc %>% autoplot(bietschhorn_wednesdays_bikes_fc, level = NULL) +
  guides(colour=guide_legend(title = "Models")) +
...
# Compare models and arrange by RMSE to select the best
fc %>% accuracy(bietschhorn_wednesdays_bikes_fc) %>% arrange(RMSE)
```
